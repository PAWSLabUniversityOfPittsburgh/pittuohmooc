...
do
    ...
    for abcd in {0..0} #0..3
    do
        lldata=${prefixR}/data/${dataset}_liblineardata/${dataset}_allsubmit_llpfa${ABCD[$abcd]}wpLN.txt
        llmodel=${prefixR}/model/${dataset}_allsubmit_llpfa${ABCD[$abcd]}wpLN_model.txt
        llpredict=${prefixR}/predict/${dataset}_allsubmit_llpfa${ABCD[$abcd]}wpLN_predict.txt
        llout=${prefixR}/out/${dataset}_allsubmit_llpfa${ABCD[$abcd]}wpLN_out.txt
	    NG=`wc -l <${prefixR}/data/${dataset}_voc/${dataset}_allsubmit_voc_student.txt`
        NK=`wc -l <${prefixR}/data/${dataset}_voc/${dataset}_allsubmit_voc_skill${ABCD[$abcd]}.txt`
        ./bin/train -q -s 14 -g 1:1-$(($NG-1)) "${lldata}" "${llmodel}" | tee "${llout}"
        ./bin/predict -b 1 "${lldata}" "${llmodel}" "${llpredict}" | tee -a "${llout}"
    done
done
