...
do
    ...
    for abcd in {0..5}
    do
		echo ${dataset} ${ABCD[$abcd]} wpN
        lldata=./data/${dataset}_liblineardata/${dataset}_allsubmit_llafm${ABCD[$abcd]}wpN.txt
        llmodel=./model/${dataset}_allsubmit_llafm${ABCD[$abcd]}wpN_model.txt
        llpredict=./predict/${dataset}_allsubmit_llafm${ABCD[$abcd]}wpN_predict.txt
