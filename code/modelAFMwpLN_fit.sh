...
do
    ...
    for abcd in {0..5}
    do
		echo ${dataset} ${ABCD[$abcd]} wpLN
        lldata=./data/${dataset}_liblineardata/${dataset}_allsubmit_llafm${ABCD[$abcd]}wpLN.txt
        llmodel=./model/${dataset}_allsubmit_llafm${ABCD[$abcd]}wpLN_model.txt
        llpredict=./predict/${dataset}_allsubmit_llafm${ABCD[$abcd]}wpLN_predict.txt
