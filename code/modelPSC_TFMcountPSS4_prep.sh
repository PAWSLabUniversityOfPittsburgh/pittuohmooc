...
do
	...
	lldata=./data/${dataset}_liblineardata/${dataset}_allsubmit_llTFMcountPSS4.txt
	export skl_col=${ABCD_column[$abcd]}
	awk -F"\t|;" 'BEGIN{fn=0;skl_col=ENVIRON["skl_col"]; nb=4;}{
		if(FNR==1) { fn++;}
		if(fn==1) {
			G[$1] = $2;
			ng++;
		...
		if(fn==4){
			...
			BG[gc,1] += $22;  # for this user
			BG[gc,2] += $23;
			BG[gc,3] += $24;
			BG[gc,4] += $25;
			BGP[gcp,1] += $22;  # since start of this problem for this user
			BGP[gcp,2] += $23;
			BGP[gcp,3] += $24;
			BGP[gcp,4] += $25;
		}
