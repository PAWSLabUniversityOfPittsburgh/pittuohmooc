...
do
    ...
    for abcd in {0..5}
    do
		echo ${dataset} ${ABCD[$abcd]}
        lldata=./data/${dataset}_liblineardata/${dataset}_allsubmit_llafm${ABCD[$abcd]}.txt
        llmodel=./model/${dataset}_allsubmit_llafm${ABCD[$abcd]}_model.txt
        llpredict=./predict/${dataset}_allsubmit_llafm${ABCD[$abcd]}_predict.txt
