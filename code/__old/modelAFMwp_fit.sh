...
do
    ...
    for abcd in {0..5}
    do
		echo ${dataset} ${ABCD[$abcd]} wp
        lldata=./data/${dataset}_liblineardata/${dataset}_allsubmit_llafm${ABCD[$abcd]}wp.txt
        llmodel=./model/${dataset}_allsubmit_llafm${ABCD[$abcd]}wp_model.txt
        llpredict=./predict/${dataset}_allsubmit_llafm${ABCD[$abcd]}wp_predict.txt
