...
do
    ...
    for abcd in {0..5}
    do
		echo ${dataset} ${ABCD[$abcd]} N
        lldata=./data/${dataset}_liblineardata/${dataset}_allsubmit_llafm${ABCD[$abcd]}N.txt
        llmodel=./model/${dataset}_allsubmit_llafm${ABCD[$abcd]}N_model.txt
        llpredict=./predict/${dataset}_allsubmit_llafm${ABCD[$abcd]}N_predict.txt
