...
do
    ...
    for abcd in {0..5}
    do
        lldata=./data/${dataset}_liblineardata/${dataset}_allsubmit_llpfa${ABCD[$abcd]}wpN.txt
        llmodel=./model/${dataset}_allsubmit_llpfa${ABCD[$abcd]}wpN_model.txt
        llpredict=./predict/${dataset}_allsubmit_llpfa${ABCD[$abcd]}wpN_predict.txt
