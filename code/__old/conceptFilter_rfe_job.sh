...
mkdir ${prefixR}/data/fc_rfe #create directory fc if not exists

for dataset in ${datasets[@]} # for all datasets
do
    mkdir -p ${prefixR}/data/fc_rfe/${dataset}_problems
    ...
    while read -r line || [[ -n $line ]]; do # for al problem files, look at "done" line below to see that we are reading a *_problem.txt file line bby line
       ...
       for abc in {0..0}
       do
            #fnfcdataAvgTs_AvgCs="${prefixR}/data/fc_rfe/"${dataset}"_problems/"$line"_"${ABC[$abc]}"_AvgTs_AvgCs_FCT_input.txt" # the data for concept selection algorithm will be stored here
            ...
            fnfcdataAvgT_AvgCs="${prefixR}/data/fc_rfe/"${dataset}"_problems/"$line"_"${ABC[$abc]}"_AvgT_AvgCs_FCT_input.txt" # the data for concept selection algorithm will be stored here
